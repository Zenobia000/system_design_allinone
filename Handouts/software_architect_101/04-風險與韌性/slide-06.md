...
## GPT Image Prompt
Create a 1920x1080 horizontal PowerPoint educational slide for "架構師 101" course. Background: Deep Navy #152238. Brand colors only: #152238, #F4F1EA, #2E7D86, #97E8D6, #E8634F, #5B9770. Top-left: "REAL WORLD" kicker pill — Forest Green #5B9770 background, Warm White text, Inter 700 / 24 px, all-caps, rounded. Title "大廠主動弄壞自己" Noto Sans TC 900 / 80 px / Warm White, left-aligned. Below: 3 body lines Noto Sans TC 500 / 34 px / Warm White, line-height 1.60. Right side (~40% canvas): two flat minimal illustration panels. Panel 1 (top): a simple node-and-line network diagram — 4 rectangular nodes (Deep Navy fill, Deep Teal #2E7D86 2 px border, JetBrains Mono Warm White labels) connected by Mint #97E8D6 2 px lines; one connecting line has a Coral Red #E8634F ✕ cut marker (intentional failure), and a curved rerouting arrow in Forest Green #5B9770 shows traffic bypassing the failure. Small "Chaos" label in JetBrains Mono 20 px Mint above the panel. Panel 2 (bottom): two zone rectangles labeled "AZ-1" and "AZ-2" in JetBrains Mono 22 px Warm White — "AZ-1" contains 2 small node stacks at 25% opacity (failed/dim), "AZ-2" contains 2 small node stacks at full brightness with Forest Green #5B9770 2 px border accent; a dashed Mint arrow flows from AZ-1 to AZ-2 indicating failover. Small "Multi-AZ" label JetBrains Mono 20 px Mint above. All flat, 2 px line-art, no 3D, no gradients, no neon, no real company logos, no photos. Bottom-right: logo placeholder 64 px (light). Footer "桑尼資料科學 · 版權所有 ©" 22 px Warm White.
...
## Speaker Notes
韌性不是紙上設計，需要主動演練。混沌工程的核心理念是：在受控的環境下，主動向系統中注入故障（殺掉某個服務實例、切斷網路連線、讓磁碟空間寫滿），觀察系統的實際行為——而不是等到生產環境真的爆炸再發現問題。這個理念在大規模分散式系統中已是業界成熟的工程實踐，不是少數公司的特異行為。多 AZ（Multi-Availability Zone）部署：雲端服務的「可用區」是實體隔離的資料中心群，電力、網路、冷卻系統各自獨立。把服務部署在至少兩個 AZ，任何單一 AZ 發生故障（停電、硬體批次故障、網路中斷），另一個 AZ 繼續服務。這是高可用架構的基礎建設選擇。注意：多 AZ 增加延遲（跨 AZ 網路 typically 1–5ms），也增加成本（跨 AZ 資料傳輸費用，複製資料多佔空間）——這是 slide-07 要打 VCRE 分的取捨決策。
...
- [ ] Body line 1: "混沌工程：故意注入故障，驗證系統韌性" — 17 chars ✓
