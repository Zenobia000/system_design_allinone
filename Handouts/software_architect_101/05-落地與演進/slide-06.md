...
- Right panel (~30% canvas width): Evolution roadmap card.
  ...
  - Row 3: `更遠：` label + `Event Sourcing / CQRS` with trigger condition, followed by two compact what-is gloss lines in Noto Sans TC 500 / 18 px / Warm White (each ≤ 18 chars): `Event Sourcing＝存每次變更事件，可回溯歷史` and `CQRS＝讀寫模型分離，各自最佳化`.
