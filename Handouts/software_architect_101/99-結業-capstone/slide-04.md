...
- Self-check Rubric (below body — structured checklist):
  ...
  **v4 · 風險與韌性**
  ...
  - □ 說出 Kafka 緩解（3-broker cluster · replication.factor=3）
  - □ Stream Processor：Consumer Group ≥2 + 冪等
...
- Self-check Rubric (below body, ~90% canvas width):
  ...
  - 2–4 checkbox items per version block (v4 has 4 to cover all three SPOFs + TSDB緩解).
...
## GPT Image Prompt
Create a 1920x1080 horizontal PowerPoint educational slide for "架構師 101" course Capstone chapter. Background: Deep Navy #152238. Brand colors only: #152238, #F4F1EA, #2E7D86, #97E8D6, #E8634F, #5B9770. Top-left: "KEY QUESTIONS" pill badge — Mint #97E8D6 background, Deep Navy #152238 text, Inter 700 / 24 px, all-caps. Title "換你默畫一次" Noto Sans TC 900 / 80 px / Warm White, left-aligned. Below title: 3 body lines Noto Sans TC 500 / 34 px / Warm White, line-height 1.60, rendered exactly as: "不看講義，默畫 v1→v5 + 每步關鍵決策" / "畫出：哪個版本加了什麼、為什麼加" / "完成後對照評分標準，看你遺漏了哪一步". Below body: a self-check rubric area (~90% canvas width). 5 compact section blocks in a horizontal strip or 2-row grid (one per version v1–v5). Each block: #172A40 background, Mint #97E8D6 1 px border, rounded 12 px. Block header: version tag in JetBrains Mono Inter 700 26 px Mint, plus chapter name in Noto Sans TC 500 22 px Warm White. Inside each block, checkbox items each start with an empty "□" in Mint followed by Noto Sans TC 500 22 px Warm White text (technical values in JetBrains Mono Mint). Render the block headers and every checkbox item exactly as the following Chinese strings:
  - Block v1 — header "v1 · 需求與約束"; items "□ 列出 NFR 矩陣（P99 < 10s / 6,000 msg/s / 99.9%）", "□ 說出三個約束（$5,000/月 / 3 個月 / 6 人）".
  - Block v2 — header "v2 · 建模與選型"; items "□ 畫出五個領域實體（Device / Sensor / Reading / Threshold / Alert）", "□ 說出選 TimescaleDB 的原因（壓縮 / retention / SQL 不變）", "□ 知道 Kafka 是為了什麼（尖峰 6,000 msg/s 削峰）".
  - Block v3 — header "v3 · 系統設計"; items "□ 畫出三條路徑（寫入 / 讀取 / 告警）", "□ 說出 Ingest API 為何回 202（非同步解耦）", "□ 說出 cache-aside 的 hit/miss 流程".
  - Block v4 — header "v4 · 風險與韌性"; items "□ 說出三個 SPOF（TSDB / Kafka / Processor）", "□ 說出 TSDB 的緩解手法（Replica + 讀寫分離）", "□ 說出 Kafka 緩解（3-broker cluster · replication.factor=3）", "□ Stream Processor：Consumer Group ≥2 + 冪等".
  - Block v5 — header "v5 · 落地與演進"; items "□ 說出可觀察性三本柱（Metrics / Traces / Logs）", "□ 說出拆微服務的觸發條件（> 20 人 / 獨立擴展需求）". Bottom-right corner: keep it clean and completely empty (reserved for a brand logo overlaid later) — do not draw any logo, emblem, badge, monogram, or icon there. Footer "桑尼資料科學 · 版權所有 ©" 22 px Warm White at bottom-left. Clean quiz/practice layout composition.
...
## Speaker Notes
最好的記憶方式是默畫。闔上講義，拿出一張空白紙或打開一個空白投影片，把 v1 到 v5 的演化畫出來，每個版本加了什麼、對應什麼決策。評分標準有 14 個打勾項，分佈在五個版本。v1 的兩個——NFR 矩陣和三個約束——如果說不清楚，代表 v1 需要複習。v3 的三條路徑（寫入 / 讀取 / 告警）是整門課最核心的架構概念，如果只記得其中兩條，回去看 slide 5 和 slide 6。v4 有四個打勾項：三個 SPOF（TSDB / Kafka / Processor）各有緩解手法——TSDB Replica、Kafka 3-broker cluster（replication.factor=3）、Stream Processor Consumer Group ≥2 + 冪等。如果只記得 TSDB 忘了 Kafka 或 Processor，回去看一遍 FMEA 表格。這不是考試——是你自己知道哪裡還需要深化的工具。
...
- [ ] Body line 1 ≤ 18 chars: "不看講義，默畫 v1→v5 + 每步關鍵決策" = 13 chars ✓
...
- [ ] v4 block: 三個 SPOF (TSDB / Kafka / Processor) + TSDB 緩解 + Kafka 緩解 (3-broker · replication.factor=3) + Stream Processor 緩解 (Consumer Group ≥2 + 冪等) — matches actual ch04 slide-05.
