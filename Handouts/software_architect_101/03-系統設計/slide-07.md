...
- VCRE Scorecard (below body, ~90% width, 4 equal cards, rounded 16 px, gap 16 px):
  ...
  - Card C: background Deep Teal `#2E7D86`. Top: `C` Inter 800 48 px Warm White. Mid: `成本 TCO` Noto Sans TC 700 28 px Warm White. Score: `3/5` JetBrains Mono 34 px Coral Red `#E8634F`. Reason: `Kafka 多一層，維運成本+1` Noto Sans TC 500 26 px Warm White.
  - Card R: background Deep Navy `#152238`, border Mint `#97E8D6` 2 px. Top: `R` Inter 800 48 px Mint. Mid: `風險` Noto Sans TC 700 28 px Warm White. Score: `4/5` JetBrains Mono 34 px Forest Green `#5B9770`. Reason: `Consumer 獨立重試，寫入隔離` Noto Sans TC 500 26 px Warm White.
...
| C 成本 TCO | 3 | Kafka 多一個中介層：額外的雲端資源（Kafka broker 叢集 ~$100–300/月）＋工程師需要學 Kafka 運維（對熟 Python 剛碰雲的 6 人團隊有學習成本）。相較同步方案 TCO 增加，但絕對金額在 $5,000/月預算內可接受。 |
| R 風險 | 4 | Queue 非同步降低了多種風險：(1) Consumer 失敗可重試，寫入不丟失；(2) Ingest API 和 TSDB 故障隔離，一邊壞不影響另一邊接收；(3) 尖峰流量不直接砸 TSDB，系統穩定性大幅提升。但同時引入新風險：Kafka 本身成為依賴（SPOF 候選，v4 處理）。整體風險改善，評 4/5。 |
...
## GPT Image Prompt
Create a 1920x1080 horizontal PowerPoint educational slide for "架構師 101" course. Background: Deep Navy #152238. Brand colors only: #152238, #F4F1EA, #2E7D86, #97E8D6, #E8634F, #5B9770. Top-left: "TRADE-OFF" dual-color pill badge — left half Deep Teal #2E7D86, right half Coral Red #E8634F, diagonal split, Warm White text, 24 px Inter 700. Title "同步還是佇列" Noto Sans TC 900 / 80 px / Warm White, left-aligned. Below: 3 body lines Noto Sans TC 500 / 34 px / Warm White, technical values (6,000 msg/s, TSDB, Kafka, Consumer) in JetBrains Mono Mint #97E8D6. Below body: VCRE scorecard — 4 equal horizontal rounded cards (16 px, gap 16 px, ~90% width). Card V (#152238 Mint 2 px border): letter V Inter 800 48 px Mint, "商業價值" 28 px Warm White, score "4/5" JetBrains Mono 34 px Forest Green #5B9770, reason line 26 px Warm White. Card C (#2E7D86): letter C Warm White, "成本 TCO", score "3/5" Coral Red. Card R (#152238 Mint border): letter R Mint, "風險", score "4/5" Forest Green. Card E (#2E7D86): letter E Warm White, "可演進", score "5/5" Forest Green. Below cards: caption "結論：Queue 非同步是正確選擇，成本是多學一層 Kafka。" Mint #97E8D6 26 px. Bottom-right: logo placeholder 64 px (light). Footer "桑尼資料科學 · 版權所有 ©" 22 px Warm White.
...
- Do not make all 4 VCRE score numbers the same color — C score is Coral Red, V/R/E scores are Forest Green.
- Do not move logo or footer outside the 96 px safe margin.
- Do not omit the conclusion caption below the VCRE cards.

## Speaker Notes
取捨的核心問題：「加 Kafka 值不值？」用 VCRE 打分。V（商業價值）4/5：削峰讓系統在尖峰 6,000 msg/s 時不崩，直接保護 $20,000/hr 的停機損失，商業價值高。C（成本 TCO）3/5：Kafka broker 的雲費加上 6 人團隊需要學習 Kafka 運維，有額外成本，但絕對金額在預算內，所以是 3 而不是 4 或 5。R（風險）4/5：Consumer 失敗可重試、Ingest 和 TSDB 故障隔離、尖峰不直接砸 DB——這三點讓整體風險大幅改善；只是 Kafka 本身成為新的依賴，但這是 v4 要處理的事。E（可演進）5/5：Queue 架構是未來加功能的底座——加 Consumer Group、多種下游處理（分析、ML pipeline）、換儲存層都不影響 Ingest——演進性滿分。結論：同步方案雖然簡單，但對這個規模的 IoT 系統，它的代價是犧牲穩定性和演進性。Queue 非同步是正確選擇，付出的代價是多學一層 Kafka。
...
- [ ] VCRE scores: V=4 (Forest Green), C=3 (Coral Red), R=4 (Forest Green), E=5 (Forest Green).
