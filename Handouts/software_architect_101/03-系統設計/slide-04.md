...
- 詞彙卡 ×3：
  ...
  - `Cache` / 快取：將熱點查詢結果暫存，避免重複打資料庫
    - cache-aside（卡內補充行）：`cache-aside：先查快取，miss 再打 DB 並回填`
  - `Message Queue` / 訊息佇列：生產者與消費者解耦，吸收尖峰流量
- 削峰因果（caption ×2 行）：
  - `同步寫：尖峰 6,000 msg/s 直接砸 DB 會飽和`
  - `佇列削峰：先進 Kafka，Consumer 慢慢消化`
- 迷你資料流（兩條路徑，節點中英對照）：
  - 寫入路徑：`Device 設備` → `Ingest API 收資料` → `Queue 佇列(Kafka)` → `Processor 消化寫入`
  - 讀取路徑：`Dashboard 儀表板` → `Query API 查詢` → `Cache 快取(60s)`
  - 圖例：`實線＝即時　虛線＝排隊`
...
## GPT Image Prompt
Create a 1920x1080 horizontal PowerPoint educational slide for "架構師 101" course. Background: Deep Navy #152238. Brand colors only: #152238, #F4F1EA, #2E7D86, #97E8D6, #E8634F. Top-left: "METHOD" kicker pill — Deep Teal #2E7D86 background, Warm White text, Inter 700 / 24 px, all-caps, letter-spacing 0.12 em. Title "削峰用佇列" Noto Sans TC 900 / 80 px / Warm White, left-aligned. Below title: three vocabulary cards arranged in a horizontal row, equal width ~530 px, height ~200 px, gap 24 px. Each card: Deep Teal #2E7D86 background, rounded corners 16 px. Each card top = English term JetBrains Mono 500 34 px Mint #97E8D6 + "/" + Chinese name Noto Sans TC 500 34 px Warm White; thin Mint 1 px horizontal divider; card bottom = one-sentence definition Noto Sans TC 500 34 px Warm White line-height 1.40. Render these EXACT three cards verbatim — Card 1 top "Stateless / 無狀態服務", definition "服務不保存請求間狀態，可任意水平擴展"; Card 2 top "Cache / 快取", definition "將熱點查詢結果暫存，避免重複打資料庫"; Card 3 top "Message Queue / 訊息佇列", definition "生產者與消費者解耦，吸收尖峰流量". Inside Card 2 (Cache), below its definition line, add one extra small line in Noto Sans TC 500 / 26 px Warm White, exactly "cache-aside：先查快取，miss 再打 DB 並回填". Between the cards and the mini-diagram strip, render a two-line "削峰因果" caption in Noto Sans TC 500 / 26 px Warm White, line 1 exactly "同步寫：尖峰 6,000 msg/s 直接砸 DB 會飽和", line 2 exactly "佇列削峰：先進 Kafka，Consumer 慢慢消化". Below the three cards: a compact mini-diagram with two clearly-labeled rows. At the left of each row, a Chinese row label in Noto Sans TC 22 px Mint #97E8D6 — top row "寫入路徑", bottom row "讀取路徑". Every mini-node is a small Deep Navy box with Mint #97E8D6 2 px border and a BILINGUAL label stacked: English on top (JetBrains Mono 20 px Warm White), Chinese below (Noto Sans TC 18 px Mint). Top row "寫入路徑": "Device 設備" → Mint SOLID arrow → "Ingest API 收資料" → Mint DASHED arrow → "Queue 佇列(Kafka)" → Mint SOLID arrow → "Processor 消化寫入". Bottom row "讀取路徑": "Dashboard 儀表板" → Mint SOLID arrow → "Query API 查詢" → Mint SOLID arrow → "Cache 快取(60s)". A short vertical dashed Mint line from Cache hints at write-back. To the right of the two rows, a tiny legend in Noto Sans TC 18 px Warm White: "實線＝即時　虛線＝排隊". Bottom-right corner: keep it clean and completely empty (reserved for a brand logo overlaid later) — do not draw any logo, emblem, badge, monogram, or icon there. Footer "桑尼資料科學 · 版權所有 ©" 22 px Warm White at bottom-left. Clean educational layout.
...
- Mini-diagram nodes must show the bilingual labels exactly (English + Chinese) and the two rows must be labeled 寫入路徑 / 讀取路徑 — do not leave nodes as bare English.
