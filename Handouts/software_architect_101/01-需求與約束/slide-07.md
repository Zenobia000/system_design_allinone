...
- Body:
  ...
  - 99.9% vs 99.99%：差 ≈ 38.7 分/月 ≈ 少損失 ~$13,000
...
- Body (3 lines): Noto Sans TC 500 / 34 px / Warm White `#F4F1EA` / line-height 1.60, left-aligned. Numbers ($20,000, $333, 99.9%, 99.99%, 38.7, ~$13,000, $2,000, $4,000) in JetBrains Mono.
- VCRE Scorecard (below body, ~90% width, 4 equal cards, rounded 16 px, gap 16 px):
  ...
  - Card C: background Deep Teal `#2E7D86`. Top: `C` Inter 800 48 px Warm White. Mid: `成本 TCO` Noto Sans TC 700 28 px Warm White. Score: `4/5` JetBrains Mono 34 px Coral Red. Reason: `月增 $2,000–4,000 + 維運人力` Noto Sans TC 500 26 px Warm White; numbers JetBrains Mono.
...
| 維度 | 評分（1-5） | 本決策的具體理由 |
|------|------------|----------------|
| V 商業價值 | 2 | 99.9%→99.99% 每月少損失約 ~$13,000，但前提是故障頻率如預期；現階段 MVP 規模未到，實際收益有限。 |
...
## GPT Image Prompt
Create a 1920x1080 horizontal PowerPoint educational slide for "架構師 101" course. Background: Deep Navy #152238. Brand colors only: #152238, #F4F1EA, #2E7D86, #97E8D6, #E8634F, #5B9770. Top-left: "TRADE-OFF" dual-color pill badge — left half Deep Teal #2E7D86, right half Coral Red #E8634F, diagonal split, Warm White text, 24 px Inter 700. Title "該追求 99.99% 嗎" Noto Sans TC 900 / 80 px / Warm White, left-aligned. Below: 3 body lines Noto Sans TC 500 / 34 px / Warm White, technical values in JetBrains Mono. Below body: VCRE scorecard — 4 equal horizontal rounded cards (16 px, gap 16 px, ~90% width), left to right exactly as follows. Card V (background #152238, Mint #97E8D6 2 px border): letter "V" Inter 800 48 px Coral Red #E8634F, name "商業價值" Noto Sans TC 700 28 px Warm White, score "2/5" JetBrains Mono 34 px Coral Red #E8634F, reason "MVP 規模未到，收益有限" Noto Sans TC 500 26 px Warm White. Card C (background #2E7D86): letter "C" 48 px Warm White, name "成本", score "4/5" JetBrains Mono 34 px Coral Red #E8634F, reason "熱備援＋多 AZ，雲費月增高". Card R (background #152238, Mint border): letter "R" Mint #97E8D6, name "風險", score "4/5" Coral Red #E8634F, reason "複雜度翻倍，新手團隊難駕馭". Card E (background #2E7D86): letter "E" Warm White, name "可演進", score "5/5" Forest Green #5B9770, reason "先穩 99.9%，v4 再演進". (Score color rule: V/E high=Forest Green good, low=Coral Red concerning; C/R high=Coral Red concerning, low=Forest Green good.) Below cards: caption "結論：v1 定 99.9%，v4 演進至 99.99%。" in Mint #97E8D6 26 px. Bottom-right corner: keep it clean and completely empty (reserved for a brand logo overlaid later) — do not draw any logo, emblem, badge, monogram, or icon there. Footer "桑尼資料科學 · 版權所有 ©" 22 px Warm White at bottom-left. Balanced decision-framework composition.
...
- Do not invent or alter the VCRE scores, dimension names, or reason text — render exactly V=2/商業價值, C=4/成本, R=4/風險, E=5/可演進 with the reason lines given.

## Speaker Notes
用 VCRE 框架來打分。V（商業價值）2/5：99.99% 每月能少損失 ~$13,000 停機費——聽起來有價值，但前提是故障真的那麼頻繁；MVP 階段用戶規模還小，這個收益是假設性的。C（成本 TCO）4/5：熱備援 + 多 AZ + 自動切換，保守估計雲費月增 $2,000–4,000，加上人力維運成本，對 $5K/月預算是很大的壓力。R（風險）4/5：複雜度翻倍，6 人 Python 新手團隊 3 個月內駕馭高可用架構的失敗風險反而很高，設計做不好可能比 99.9% 還不穩。E（可演進）5/5：先做好 99.9% 打穩 v1 基礎，v3 系統設計完、v4 加韌性模式之後再演進至 99.99%，每步都是安全的增量改進。結論：v1 先定 99.9%，這不是認輸，是務實的分階段決策。
...
- [ ] Body line 2: "99.9% vs 99.99%：差 ≈ 38.7 分/月 ≈ 少損失 ~$13,000" ≤ 18 chars ✓
