...
市面上的產品框架知識庫已經很多了 — 多數是「精選 N 個框架」的網格，告訴你**有哪些工具**。

但工程團隊真正需要的不是更多框架，而是一條**可走完的路** — 從一個商業假設，走到一個**凌晨三點還活著的系統**。

**Launch Atlas** 不只列工具，而是把工具放回它「該被使用」的位置——告訴你「下一步該做什麼、誰該做、AI 怎麼幫你做完」。
...
相較於市面上常見的「框架精選網格」，Launch Atlas 做了三個關鍵翻轉：

| 維度 | 常見框架知識庫 | Launch Atlas |
|---|---|---|
| **內容單位** | 精選 N 個框架 | **54 個交付物**（PRD/ADR/SRS/Runbook/SLO） |
| **結構** | 單長頁 + filter | **長頁 + 三層子頁**（角色 / 階段 / 交付物） |
| **視覺** | 偏向光面紙質感 | **深墨 hero + 米白內容**雙主題 |
| **每張卡** | 一句話框架描述 | **四問結構**：解決什麼 / 誰負責 / 何時用 / AI 怎麼加速 |
| **哲學** | 列出有哪些工具 | **不背名詞學決策 + Why/How/Trade-off + AI 加速三問** |
